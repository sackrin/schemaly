...
export class Depends implements Condition {
  ...
  public check = ({
    ...
    return this.checks.reduce((_curr: boolean, checker) => {
      // Trigger and wait for the checker to return a value
      // Checkers can be promises to allow for dynamic checks
      const result = checker(collider, blueprint, hydrate, options);
