...
import {COLLECTION, CONTAINER, Field, Fields, Nucleus, STRING} from "../../Nucleus";
...
describe("Isotope/Hydrate", (): void => {
  ...
  const mockSingleParams = {
    ...
    nucleus: Field({
      ...
      setters: [
        ({ isotope, value }: { isotope: Isotope, value: any }) => (value.toString().toUpperCase()),
      ],
      getters: [
        ({ isotope, value }: { isotope: Isotope, value: any }) => (value.toString().toUpperCase()),
      ],
  ...
  it("can create an isotope instance from a nucleus", () => {
    const isotope = getHydrate({ ...mockSingleParams, options: { testing: true } });
  ...
  it("can hydrate an isotope against a container set", () => {
    ...
    .then(() => {
      ...
      expect(fakeIsotope.find({ machine: "first_name" })).to.have.property("value", "Toby");
      expect(fakeIsotope.find({ machine: "surname" })).to.have.property("value", "Smith");
    }).catch((msg) => {
      throw new Error(msg);
    });
  });
  ...
  it("can use the find method on a repeater", () => {
    ...
    .then(() => {
      ...
      expect(fakeIsotope.find({ machine: "label" })).to.have.property("value", "Home Address");
      expect(fakeIsotope.find({ machine: "address" })).to.not.be.undefined;
      expect(fakeIsotope.find({ machine: "address" })).to.have.property("value", "test1@example.com");
      expect(fakeIsotope.find({ machine: "secret" })).to.be.undefined;
      expect(fakeIsotope.find({ machine: "notexists" })).to.be.undefined;
    }).catch((msg) => {
      throw new Error(msg);
  ...
  it("can use the filter method on a container", () => {
    ...
    .then(() => {
      ...
      expect(fakeIsotope.filter({ machine: "first_name" })[0]).to.have.property("value", "Toby");
      expect(fakeIsotope.filter({ machine: "surname" })).to.have.length(1);
      expect(fakeIsotope.filter({ machine: "surname" })[0]).to.have.property("value", "Smith");
      expect(fakeIsotope.filter({ machine: "secret" })).to.have.length(0);
  ...
  it("can use the filter method on a collection", () => {
    ...
    .then(() => {
      expect(fakeIsotope.filter({ machine: "label" })).to.have.length(2);
      expect(fakeIsotope.filter({ machine: "label" })[0]).to.have.property("value", "Home Address");
      expect(fakeIsotope.filter({ machine: "address" })).to.have.length(2);
      expect(fakeIsotope.filter({ machine: "address" })[0]).to.have.property("value", "test1@example.com");
      expect(fakeIsotope.filter({ machine: "secret" })).to.have.length(0);
      expect(fakeIsotope.filter({ machine: "notexists" })).to.have.length(0);
    }).catch((msg) => {
      throw new Error(msg);
  ...
  it("can sanitize a simple isotope", () => {
    const fakeIsotope = getHydrate({ ...mockSingleParams, value: "  johNSton " });
    return fakeIsotope
    .hydrate()
  ...
  it("can sanitize a container isotope", () => {
    ...
    .then(fakeIsotope.sanitize)
    .then(() => {
      expect(fakeIsotope.find({ machine: "first_name" })).to.have.property("value", "JESSICA");
      expect(fakeIsotope.find({ machine: "surname" })).to.have.property("value", "SMITHERSON");
    }).catch((msg) => {
      throw new Error(msg);
    });
  });
  ...
  it("can sanitize a repeater isotope", () => {
    ...
    .then(() => {
      expect(fakeIsotope.filter({ machine: "label" })[0]).to.have.property("value", "HOME ADDRESS");
      expect(fakeIsotope.filter({ machine: "address" })[0]).to.have.property("value", "BILL@EXAMPLE.COM");
      expect(fakeIsotope.filter({ machine: "label" })[1]).to.have.property("value", "WORK ADDRESS");
      expect(fakeIsotope.filter({ machine: "address" })[1]).to.have.property("value", "JOHN@EXAMPLE.COM");
    }).catch((msg) => {
      throw new Error(msg);
    });
  });
